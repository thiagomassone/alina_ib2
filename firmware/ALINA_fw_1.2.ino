...
const float T12_PITCH_MIN =  -4.5;

// ─── Parámetros de ventana (NUEVOS) ──────────────────────────────────────────
// Ventana DESLIZANTE de cada 15s, umbral de proporción 60%.
const unsigned long VENTANA_MS    = 15000;  // ventana deslizante analizada
...
// Buffer circular: 15s a 10Hz = 150 muestras. Guarda mala/buena por punto.
const int BUF_N = (int)(VENTANA_MS / PERIODO_MS);   // 150
bool bufMalaT1[BUF_N];
bool bufMalaT12[BUF_N];
...
void procesarComando(const String &msg) {
  ...
  if (strcmp(cmd, "start_session") == 0) {
    ...
    // Calibración por app: respeta el mínimo de 5s igual que el botón.
    // 1ra llamada arranca; 2da llamada (≥5s después) confirma y guarda.
    ...
    // (procesarBoton ya guarda RPSIS internamente al confirmar)
...
void dispararVibracion(bool grupoT12, bool grupoT1) {
  ...
  vibrando = true;
  // "Intensidad" háptica = duración del pulso (los motores son ON/OFF por GPIO).
  unsigned long dur = (cfgHapticIntensity > 0) ? (unsigned long)cfgHapticIntensity : VIBRA_MS;
  tFinVibra = millis() + dur;
  alertasHapticas++;
}

// T1: mala si pitch < -11.2  O  |roll| > 22.4
bool malaPosturaT1(float p, float r)  { return (p < T1_PITCH_MIN)  || (fabs(r) > T1_ROLL_LIM); }
// T12: mala SOLO por pitch < -4.5 (el roll se mide pero no clasifica)
bool malaPosturaT12(float p, float r) { (void)r; return (p < T12_PITCH_MIN); }
...
void resetBuffer() {
  ...
  for (int i = 0; i < BUF_N; i++) { bufMalaT1[i] = false; bufMalaT12[i] = false; }
}
...
void procesarBoton(float t1p, float t1r, float t12p, float t12r) {
  ...
  if (estadoCalib == SIN_CALIBRAR || estadoCalib == CALIBRADO) {
    // 1ra pulsación: borra la calibración vieja y arranca una nueva.
    estadoCalib  = ESPERANDO_FIN;
    tInicioCalib = ahora;
    if (sdDisponible && SD.exists("/calibracion.txt")) SD.remove("/calibracion.txt");
    Serial.println(">> PRIMERA PULSACIÓN. Mantené la postura y esperá más de 5s antes de pulsar de nuevo.");
  } else if (estadoCalib == ESPERANDO_FIN) {
    if (ahora - tInicioCalib >= CALIB_MIN_MS) {
      // Guardar referencia de los 3 puntos (incluye RPSIS como referencia)
      ...
      resetBuffer();   // arranca limpio para no arrastrar muestras viejas
      guardarCalibracionSD();
      wsEnviarStatus();
      Serial.printf(">> SEGUNDA PULSACIÓN. Calibración COMPLETA y guardada. Pasaron %.1fs.\n",
                    (ahora - tInicioCalib) / 1000.0);
      Serial.println("Calibración postural completada y guardada.");
    } else {
...
void loop() {
  unsigned long ahora = millis();
  webSocket.loop();
  // LED: titila mientras espera el fin de calibración, fijo apagado si no
  if (estadoCalib == ESPERANDO_FIN) {
    digitalWrite(LED_PIN, (millis() / 250) % 2);   // parpadeo cada 250ms
  } else {
    digitalWrite(LED_PIN, LOW);
  }
  ...
  if (tickMuestreo) {
    ...
    if (estadoCalib == CALIBRADO) {
      ...
      // Escribir en el buffer deslizante
      bufMalaT1[bufIdx]  = malaPosturaT1(t1p, t1r);
      bufMalaT12[bufIdx] = malaPosturaT12(t12p, t12r);
  ...
  // ── 4) Evaluación deslizante cada 15s sobre los últimos 15s ──────────────
  if (estadoCalib == CALIBRADO && (ahora - tUltimaEvalT12 >= EVAL_CADA_MS)) {
    ...
    Serial.printf("[T12] ventana 15s: %d malas / %d total (%.0f%%) → %s\n",
  ...
  if (estadoCalib == CALIBRADO && (ahora - tUltimaEvalT1 >= EVAL_CADA_MS)) {
    ...
    float prop = (bufLlenado > 0) ? (float)malas / bufLlenado : 0.0;
    Serial.printf("[T1 ] ventana 15s: %d malas / %d total (%.0f%%) → %s\n",
                  malas, bufLlenado, 100.0 * prop,
